...
function exportMember(
  ...
  outMember.typeList = mergeDuplicateTypes(member.getTypes()).map((type: types.TypeBase) => {
...
function exportType(type: types.TypeBase, namespace: schema.Namespace, context: schema.Context) {
  ...
  if (listType) {
    for (var spec of listType) {
      ...
      outMember.typeList = [exportType(spec.item as types.TypeBase, namespace, context)]
